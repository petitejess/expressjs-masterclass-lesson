...
async function signUpUser(userDetails) {
  return firebaseAdmin.auth().createUser({
    ...
    .then(async (userRecord) => {
      ...
      let defaultUserClaims = firebaseAdmin.auth().setCustomUserClaims(userRecord.uid, {admin: false, regularUser: true});
    })
    .then(() => {
    ...
    .catch(err => {
      console.log(`Internal sign-up function error: ${err}`);
      return { error: err };
    });
}
